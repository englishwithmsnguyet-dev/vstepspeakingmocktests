...
def format_vietnamese(text):
    ...
    blues = [
        ...
        "sử dụng xe buýt để di chuyển quanh thành phố", "dễ sử dụng", "có sẵn ở nhiều khu vực", "chọn chúng cho những chuyến đi ngắn",
        ...
        "cho tôi sự tự do hơn khi đi lại", "không phải tuân theo một lịch trình cố định", "chờ phương tiện công cộng", "tiết kiệm rất nhiều thời gian",
        "tiết kiệm tiền", "giảm tình trạng tắc đường", "bảo vệ môi trường",
        "tiết kiệm tiền chi phí đi lại", "giảm ùn tắc giao thông",
        ...
        "hoạt động mang tính xã hội", "gặp gỡ những người có cùng sở thích tiếng Anh", "học hỏi lẫn nhau", "chia sẻ kinh nghiệm hữu ích", "duy trì động lực học tiếng Anh",
        "thực hành nói tiếng Anh", "cải thiện kỹ năng phát âm và giao tiếp", "tự tin hơn khi sử dụng tiếng Anh",
        "gặp gỡ những người có cùng sở thích", "chia sẻ kinh nghiệm học tập", "thảo luận về các chủ đề khác nhau", "duy trì động lực", "cải thiện tiếng Anh của mình hiệu quả hơn",
        "giảm ô nhiễm", "sử dụng ít nhựa hơn", "tái chế nhiều rác thải hơn", "môi trường có thể trở nên sạch sẽ hơn",
        "làm công việc tình nguyện", "giúp đỡ các gia đình khó khăn", "hỗ trợ các hoạt động cộng đồng", "quan tâm và hỗ trợ hơn",
        "tuân thủ luật pháp và các quy định", "duy trì trật tự và sự an toàn trong xã hội", "tận hưởng chất lượng cuộc sống tốt hơn",
        "tái chế rác thải", "sử dụng các sản phẩm thân thiện với môi trường", "sạch và khỏe mạnh hơn",
        "giúp đỡ những người khó khăn", "cải thiện cuộc sống", "củng cố các cộng đồng",
        "tuân theo pháp luật", "trở nên có tổ chức và ổn định hơn", "sống trong một môi trường an toàn hơn",
        "ô nhiễm không khí", "vấn đề môi trường nghiêm trọng nhất", "ảnh hưởng đến sức khỏe của con người", "chất lượng cuộc sống", "không khí không được sạch sẽ",
        "ô nhiễm nguồn nước", "ảnh hưởng đến sức khỏe con người", "phá hủy các hệ sinh thái", "đối mặt với những thách thức về môi trường",
        "cả hai nên nhận trách nhiệm", "tạo ra các luật pháp và chính sách", "tuân thủ các quy tắc", "đóng góp cho xã hội",
...
# Extra fix for "một cách" and "một số cách" which might get partially highlighted
html_output = html_output.replace('một <strong style="color: #ee0000;">cách</strong>', 'một cách')
html_output = html_output.replace('một số <strong style="color: #ee0000;">cách</strong>', 'một số cách')

with open(html_path, 'w', encoding='utf-8') as f:
    f.write(html_output)

print("Successfully fixed highlights in test11-index.html")
