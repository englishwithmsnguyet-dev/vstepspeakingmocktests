...
test4_dir = os.path.join(root_dir, 'test 4')
html_path = os.path.join(test4_dir, 'test04-index.html')

# 1. Copy app.js and styles.css
shutil.copy(os.path.join(root_dir, 'test 1', 'app.js'), os.path.join(test4_dir, 'app.js'))
shutil.copy(os.path.join(root_dir, 'test 1', 'styles.css'), os.path.join(test4_dir, 'styles.css'))
...
# 3. Rebuild answers
with open('test4_format.txt', 'r', encoding='utf-8') as f:
    lines = f.readlines()
...
print(f"Extracted {len(answers)} answers from test4_format.txt")
...
def color_vietnamese(match):
    ...
    for kw in red_keywords:
        ...
        trans_content = trans_content.replace(f' {kw}</strong>', f' <strong style="color: #ee0000;">{kw}</strong>')
...
print("Successfully updated test04-index.html")
